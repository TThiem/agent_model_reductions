...
class TwoCondEqnSim:
    ...
    def __init__(self, N=12, Iapp=None, A=None, Vsyn=0, VNa=50):
...
class DataGenerator:
    ...
    def __init__(self, N=1, rtol=1e-3, atol=1e-5, **kw):
        ...
        self.simulator.integrate(X0=np.reshape([-20, 0.4] * N, (N, 2)).T.ravel(), t_span=[0, 100], rtol=rtol, atol=atol)
    ...
    def _doWork(self, seed):
        ...
        if np.random.uniform() < self.rawProb:
            ...
            print(states.shape)
            states, times = self.simulate(X0=states[-1]+np.random.normal(size=(2,))*np.array([.1, .01]))
